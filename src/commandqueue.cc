...
JS_METHOD(CommandQueue::enqueueNDRangeKernel)
{
  ...
  cl_event event;
  if(args[5]->IsUndefined()) event=NULL;
...
JS_METHOD(CommandQueue::enqueueWriteBuffer)
{
  ...
  if(event) {
    Event *e=ObjectWrap::Unwrap<Event>(args[4]->ToObject());
    e->setEvent(event);
  }
  return Undefined();
}
...
JS_METHOD(CommandQueue::enqueueCopyBuffer)
{
  ...
  cl_event event;
  if(args[3]->IsUndefined()) event=NULL;
...
JS_METHOD(CommandQueue::enqueueWriteImage)
{
  ...
  cl_event event;
  if(args[8]->IsUndefined()) event=NULL;
...
{   
  ...
  cl_event event;
  if(args[6]->IsUndefined()) event=NULL;
...
JS_METHOD(CommandQueue::enqueueMarker)
{
  ...
  cl_event event;
  if(args[1]->IsUndefined()) event=NULL;

  cl_int ret = ::clEnqueueMarker(cq->getCommandQueue(),event==NULL ? NULL : &event);
